...
classes 8--9 are muscle outputs.  The neurons are AS, DA, DB (dorsal excitatory cholinergic),
VA, VB (ventral excitatory cholinergic), DD, VD (GABAergic inhibitory), and dorsal/ventral muscle
cells.  The class-level connectivity extracted from the six-segment connectivity matrices is
summarised in Table~\ref{tab:connectivity}.
...
\toprule
Class & $N$ & \multicolumn{1}{c}{Likely ID} & Exc.\ output & Inh.\ output & GJ partners & Inh.\ input \\
\midrule
1 & 12 & VA/VB/AS & 2,4,5,8 & --- & 2, 7 & \textbf{none} \\
2 &  6 & VB/DA    & 3,4,5,8 & --- & 1, 7 & \textbf{none} \\
3 &  6 & DA/AS    & 1,4,5,8 & --- & 3, 6 & \textbf{none} \\
4 &  6 & DD       & ---      & 5, 8 & 4, 5 & \textbf{none} \\
5 & 12 & VD       & ---      & 6,7,9 & 4,5,7 & class~4 \\
6 & 12 & DA/DB    & 4,5,7,9 & --- & 3, 6 & class~5 \\
7 & 12 & DB/VA    & 4,5,6,9 & --- & 1,2,5 & class~5 \\
8 & 18 & muscle   & ---      & --- & --- & class~4 \\
9 & 18 & muscle   & ---      & --- & --- & class~5 \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Class~5 (VD, 12 cells)}: inhibits classes~6, 7 (motor neurons) and muscle
...
\begin{itemize}
    \item \textbf{Group~A} (classes 1, 2, 3): drive muscle group~8; excite inhibitory class~4.
          Receive \emph{no direct inhibitory synaptic input}.
    \item \textbf{Group~B} (classes 6, 7): drive muscle group~9; excite inhibitory class~5.
          Receive \emph{direct inhibitory input from class~5 (VD)}.
\end{itemize}

This asymmetry is biologically faithful: in \textit{C.~elegans}, VD neurons are known to directly
inhibit dorsal motor neurons in addition to providing cross-inhibition to muscles, while DD neurons
primarily inhibit the opposite side's muscles rather than the ventral motor neurons themselves.
...
\begin{align*}
    \text{Class~1} &\xrightarrow{\mathrm{GJ}} \text{Class~7} \xleftarrow{\mathrm{inh}} \text{Class~5 (VD)} \\
    \text{Class~2} &\xrightarrow{\mathrm{GJ}} \text{Class~7} \xleftarrow{\mathrm{inh}} \text{Class~5 (VD)} \\
    \text{Class~3} &\xrightarrow{\mathrm{GJ}} \text{Class~6} \xleftarrow{\mathrm{inh}} \text{Class~5 (VD)}
\end{align*}
...
Class~3 is gap-junction coupled only to itself (intra-class, weight 10) and to class~6 (weight 5).
The GJ weight from class~6 to class~3 is the smallest single inter-class GJ weight in the network.
If class~6 is successfully returned to rest by VD, the coupling onto class~3 must still be strong
enough to drag it below $T$.  Class~3 is therefore the cell most likely to remain latched at the
plateau when the rest of the network returns to rest.
